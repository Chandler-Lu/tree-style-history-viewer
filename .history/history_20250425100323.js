  ...
  const picker = new Litepicker({
      ...
      setup: (picker) => {
          ...
          picker.on('selected', (date1, date2) => {
              if (date1 && date2) {
                  console.log('[DEBUG] Litepicker raw dates:', date1, date2); // Debug Step 1

                  // --- 使用更可靠的本地日期格式化 ---
                  const formatDateLocal = (litepickerDate) => {
                      if (!litepickerDate) return '';
                      const d = litepickerDate.toJSDate(); // 获取 JS Date 对象
                      console.log(`[DEBUG] JS Date for ${litepickerDate}:`, d); // Debug Step 2
                      // getFullYear, getMonth, getDate 基于本地时区解释 Date 对象
                      const year = d.getFullYear();
                      const month = String(d.getMonth() + 1).padStart(2, '0'); // 月份从 0 开始，需要 +1
                      const day = String(d.getDate()).padStart(2, '0');
                      const formatted = `${year}-${month}-${day}`;
                      console.log(`[DEBUG] Formatted Date String:`, formatted); // Debug Step 3
                      return formatted;
                  };
                  ...
                  console.log('[DEBUG] Set Input Values:', startDateInput.value, endDateInput.value); // Debug Step 4

...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const picker = new Litepicker({
      element: document.getElementById('datepicker-container'), // 容器元素
      inlineMode: true,           // 内联显示，而不是弹出
      singleMode: false,          // 范围选择模式
      numberOfMonths: 1,          // 显示一个月
      numberOfColumns: 1,         // 显示一列
      format: 'YYYY-MM-DD',       // 日期格式
      showTooltip: true,
      autoApply: true,            // 选择结束日期后立即应用
      startDate: new Date(new Date().setDate(new Date().getDate() - 1)), // 默认开始日期：昨天
      endDate: new Date(),        // 默认结束日期：今天
      setup: (picker) => {
          // 当日期范围被用户选择并应用后触发
  ...
  // **初始加载历史记录**
  // 手动设置初始日期到隐藏的 input，以便第一次加载使用默认值
  const formatDate = (date) => date.toISOString().split('T')[0];
